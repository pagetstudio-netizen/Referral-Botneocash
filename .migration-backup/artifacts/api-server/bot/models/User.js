...
class UserRecord {
  constructor(row) {
    ...
    this.balance = parseFloat(row.balance) || 0;
    this.referralCode = row.referral_code;
    this.referredBy = row.referred_by ? Number(row.referred_by) : null;
    this.referralCount = Number(row.referral_count) || 0;
    this.referralEarnings = parseFloat(row.referral_earnings) || 0;
    this.bonusEarnings = parseFloat(row.bonus_earnings) || 0;
    this.totalWithdrawn = parseFloat(row.total_withdrawn) || 0;
